...
export default function AdminDashboardPage() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-[var(--foreground)] mb-8">대시보드 개요</h1>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
            ...
            <span className="text-4xl font-bold text-[var(--premium-gold)]">{stats.pendingAnswers}건</span>
          </div>
          
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
            <span className="text-gray-500 font-medium mb-2">총 상담 접수 건수</span>
            <span className="text-4xl font-bold text-[var(--foreground)]">{stats.totalQuestions}건</span>
          </div>
          
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col">
            <span className="text-gray-500 font-medium mb-2">총 가입 회원 (카카오)</span>
            <span className="text-4xl font-bold text-[var(--foreground)]">{stats.totalUsers}명</span>
          </div>
        </div>
      )}

      <div className="mt-12 bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
        <h2 className="text-xl font-bold text-[var(--foreground)] mb-4">환영합니다!</h2>
